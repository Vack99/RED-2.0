...
> **✅ §B3 SUPERSEDED by #75 (walk green 2026-07-09) — and RETAINED deliberately.** The Send Email Hook now renders every signup-confirmation + password-reset mail (gym-branded, link minted on the gym's own host); these two templates no longer fire while the hook is enabled. They are kept on purpose as the **rollback rendering**: disabling the hook resumes SMTP + these templates instantly (Supabase offers no template delete, only reset-to-default — resetting would make rollback ship English defaults). Owner decision, #75 close-out.
