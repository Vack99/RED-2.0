...
## Note — 2026-08-13 (issue #269, payment correction): one carve-out from the INVOKER posture

`eliminar_venta` ships **`SECURITY DEFINER`**, the first money-path write to depart from the "SECURITY INVOKER, RLS is the authorization boundary" clause above. `editar_venta`, shipped alongside it, stays **INVOKER** under a new `ventas_staff_update` policy — the posture is unchanged for it.

> **Corrected 2026-08-28:** `editar_venta` is **no longer INVOKER** — `20260825145937_staff_gym_tenant_in_effect.sql` re-emitted it as **`SECURITY DEFINER`** (its in-body `staff_gym()` / `is_staff_of(p_gym_id)` gate is now the authorization boundary, so a supplied `p_gym_id` is checked explicitly rather than by RLS), and `20260828110000_editar_venta_as_if_original.sql` keeps that posture. Every "INVOKER" said of `editar_venta` below is historical; the `ventas_staff_update` column-granular policy still stands and still caps a raw PATCH, so the residual argued in this note is unchanged.

**Why the carve-out.** Supabase's project scaffold runs a schema-wide `ALTER DEFAULT PRIVILEGES … GRANT ALL ON TABLES TO anon, authenticated, service_role`, so `authenticated` already holds table-wide UPDATE/DELETE on `public.ventas` — inert only while no UPDATE/DELETE policy exists to compose with it (measured live on `gym` in `20260808130000_gym_legal_name_staff_update.sql:20-34`, whose header states the obligation for the next write policy on any table). `eliminar_venta` carries two rules that exist nowhere else: the 30-day window from `created_at` and the saldo clawback. Adding a `ventas_staff_delete` policy would activate the ambient grant and make a raw `DELETE /rest/v1/ventas?id=eq.<x>` a second door that skips both, leaving `clases_restantes`/`vence` permanently wrong. **DELETE has no column granularity**, so unlike UPDATE it cannot be narrowed to a safe subset — an INVOKER+policy design is unclosable here. So: no delete policy, `revoke delete on public.ventas from anon, authenticated`, and DEFINER with the gym gate (`staff_gym()` + `where id = p_venta_id and gym_id = v_gym`) inside the body. The function is the only door by construction, not by policy.

**`editar_venta` needs no carve-out** precisely because UPDATE *does* have column granularity: `revoke update on public.ventas from anon, authenticated` then `grant update (monto, metodo) on public.ventas to authenticated` caps a raw PATCH at those two columns — `folio`, `clases`, `gym_id` and `fecha` stay unreachable — and `ventas_metodo_check` still binds the método domain. The residual, stated plainly rather than claimed away: PostgREST PATCH is **filter-based**, so a raw `?cliente_id=eq.<x>` rewrites every matching row at once where the RPC touches exactly one by id; and `monto` carries **no table CHECK**, so its 1..100 000 bound exists only inside `editar_venta`. A raw PATCH is therefore *narrower in columns* but *not equivalent* to the RPC. What remains is gym-staff-scoped self-harm — a signed-in operator mangling their own gym's amounts, with RLS still pinning the tenant — accepted under the gym's-data ruling (it is the gym's data; we optimize for the administrator's agency and do not add locks the owner did not ask for). INVOKER + `ventas_staff_update` therefore holds the line.

**What stays true:** `set search_path to ''` and EXECUTE-to-`authenticated`-only on both. The definer's authorization is not weaker, only relocated — from RLS into an explicit `staff_gym()` gate, the same shape `obtener_identidad_legal` and `aceptar_acuerdo` already use where RLS cannot express the rule. Live migration: `supabase/migrations/20260813120000_editar_eliminar_venta.sql`.

**Amended 2026-08-14 (#269 fast-follow):** `fecha` added to the column grant so `editar_venta(p_fecha)` (INVOKER) can write it; the RPC's bounds (not future / ≥ alta / ≤30d back) live only on the RPC door — a raw PATCH can move `fecha` unbounded, same accepted gym's-data residual as `monto`.

**Amended 2026-08-14, later the same day (owner ruling, recorded on #266):** the **alta floor is dropped at both doors** (`registrar_venta` backdate and `editar_venta(p_fecha)`) — gyms register walk-in clients late, so `clientes.created_at` is a data-entry stamp, not the arrival date, and the floor blocked exactly the forgotten-sale correction it was meant to serve. The RPC bounds are now: not future / ≤30 days back. Migration: `20260814130000_drop_alta_floor.sql`.
