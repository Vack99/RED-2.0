...
- **The invitation link now targets the activation door (`/activar`), not the self-registration form (`/registro`).** Same host-mapped-per-gym URL rule and `?gym=` fallback host; only the path changes. The legacy `/registro?codigo=` claim arm has since been removed (2026-07-24, H2v2 option b) — `/activar` is the sole invite door; `/registro` remains plain host-scoped self-registration.
