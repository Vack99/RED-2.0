...
## 2026-06-02 — refining a sibling Goal-B audit skill (`to-map`/`to-findings`): skill-building & subagent-test learnings

**Scope note (read first).** This session did NOT touch Forge or the back-half shipping skill — it deepened the `to-map` → `to-findings` query-performance audit pipeline (a sibling of `improve-codebase-architecture` / `improve-database-architecture`, living uncommitted in `~/.claude/skills/`; per-task detail in the `to-map-to-findings-audit` memory + its dedicated handoff). Logged here per this ledger's "append every session" protocol. The lessons target **`write-a-skill`'s testing methodology**, the **§7 audit/re-audit machinery**, and **orchestration economics** — the harness-as-*product*, not shipping-slice content. Natural homes when the registry is next synthesized: registry §7 (audit machinery) + Operating discipline §6/§8. ([[to-map-to-findings-audit]])

**TDD-for-skills when the test subagent is itself a capable model (the headline):**
- **A strong test subagent OBSCURES RED for any judgment-codifying edit.** Codifying senior-obvious rules into a detector (e.g. "a date-window-only bound is volume-dependent MED, not safe"; "a caller-controlled `.limit(x)` is effectively unbounded"), the first RED run came back ALL-PASS: the new GREEN cases passed against the UN-edited skill, because (a) the prompts leaked the answer and (b) the test subagents are full Opus that already apply the judgment from first principles. The writing-skills Iron Law ("watch it fail first") was unsatisfiable as written — a GREEN-on-actual-file test passes with OR without the edit, proving nothing. *Why it matters:* you can fool yourself that a codification "works" when the model would have produced it anyway. *Skill implication (`write-a-skill`):* (1) be honest that such an edit is **codification** (for determinism / weaker models / drift / auditability), NOT a behavior fix; (2) prove the text change is consequential with an **old-rule-inline RED contrast** — pin a subagent to the LITERAL old-rule text and show it yields the old outcome, while the edited file yields the new one (the suite's C4/C5/C6 pattern). The literal-old↔edited flip is the honest evidence, not a GREEN a capable model passes regardless. ([[forge-validate-before-codify]])

**Subagent-test hygiene — three ways a test silently lies (all invisible until you read the subagent's raw output):**
- **Leaky prompts pass regardless of the skill.** A prompt that states the expected classification measures the model's agreement with a leading question, not whether the skill teaches the rule. *Skill implication:* write NEUTRAL prompts — give the call-site/fixture, never the answer.
- **Assert on STRUCTURED return fields, never regex over the subagent's prose.** An assertion `!/limit-dynamic/.test(notes)` FALSE-FAILED because the subagent's *correct* explanation said "there is no `limit-dynamic` concept" — the flag NAME appeared in prose explaining the flag was absent. Fixed by returning an explicit `{limitDynamic: boolean}`. *Skill implication:* a skill's test schema should expose a boolean/enum to assert on; free-text reasons are for humans, not assertions.
- **Subagents escape the fixture sandbox.** One read the real app file (`asistencia.ts`) instead of the provided call-site, polluting the verdict. *Skill implication:* every test prompt carries a hygiene line — "judge ONLY from the detector + the provided call-site; do not read other repository files."
- **A failing case is often a TEST bug, not a skill bug — read the reasoning before touching the skill.** Twice this session a red case was a contaminated control (a negative-control RSC leaf that was ALSO `.map`'d elsewhere, so the detector's codebase-wide rule *correctly* flagged it) or the brittle assertion above — the skill was right both times. *Skill implication:* root-cause a red from the subagent's own reasoning first; "fix the skill" is the last resort, not the first.

**Skeptical multi-lens audit to decide whether to improve a skill AT ALL (extends §7 to skills themselves):**
- **A dedicated anti-overengineering lens + adversarial verification answers "improve or gold-plate?" honestly.** Asked whether further work was overengineering, ran a 5-lens audit workflow (coverage / recall / portability / usability + a **skeptic lens whose job is to argue for REMOVAL**), each candidate verified as genuine / gold-plating / already-covered. Result: 9 genuine gaps, 7 gold-plating, 2 already-covered — and the skeptic's 5 removal proposals were ALL rejected by the verifier, so nothing was cut. Verdict: the skill was GOOD / not over-engineered. *Why it matters:* "should I keep improving X?" is a real fork; a skeptic lens finds genuine gaps a single pass misses AND guards against manufacturing busywork. *Skill implication:* §7's machinery should ship an explicit **anti-overengineering skeptic lens**, and route ITS removal proposals through the same adversarial verifier — a removal is a claim to verify, not act on (mirrors Operating discipline §6, validate-the-validators). Applies to auditing a SKILL, not just app code.
- **Audit a multi-stage pipeline for SEVERED inter-stage handoffs.** The single highest-value finding was an end-to-end break: `/to-findings` told the user to run `/to-prd`, but `/to-prd` had no path to ingest the findings doc — the pipeline's last hop silently dropped everything (verbatim user stories, ordering, accept/reject). *Skill implication:* for any producer→consumer skill chain, add a lens that verifies the consumer can actually INGEST the producer's documented output (path arg / discovery + fallback) — a green producer + a green consumer can still be a broken pipeline.

**Orchestration economics of subagent-driven test suites:**
- **Full runs are expensive; resume on test-only changes.** Each full run of the subagent-driven regression suite cost ~450–800k subagent tokens (scales with case count). `resumeFromRunId` re-runs ONLY changed cases (~tens of k) — used it to verify single-case fixes; reserved a FULL run for when a detector/SKILL file changed (must re-validate ALL cases against the new rule). *Skill implication:* bake the **resume-vs-full-run rule** into any subagent-driven harness `write-a-skill` ships — full when the artifact-under-test changes, resume when only tests/fixtures change.
- **Partition implementation across agents by DISJOINT file sets** (producer `.md`s / the detector / the test suite ran as parallel agents that never touched the same file; surgical judgment edits done inline or via exact-string specs). Reinforces Operating discipline §8 — file-overlap graph: serialize colliding work, parallelize disjoint.

Process posture this session re-confirmed the standing directives applied to *skill-refinement*: orchestrate-don't-over-checkpoint (delegate the file-heavy work, checkpoint only genuine forks, lead with a senior recommendation), validate-before-codify, good-enough-not-perfection. One more: **version-controlling a deployed-in-place skill is hygiene, not overengineering — but a real scope decision** (dedicated repo for the two skills vs. entangling the rest of `~/.claude/skills/`), left to the operator, not silently executed. ([[forge-orchestrate-dont-over-checkpoint]], [[forge-good-enough-not-perfectionist]])
