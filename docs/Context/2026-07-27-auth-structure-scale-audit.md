...
- **Authorization never comes from the account, and never from the host.** All 25 gym-scoped SELECT policies key on `gym_id IN (SELECT m.gym_id FROM gym_membership m WHERE m.user_id = (SELECT auth.uid()))` or `is_staff_of(gym_id)`. **Not one policy reads a header.** `resolveTenant` stamps `x-gym`/`x-brand` for presentation only. ~~A global identity therefore buys an attacker nothing: `auth.uid()` alone grants zero rows anywhere.~~
  ...
  > **STRUCK 2026-08-02 — measured false.** A fabricated `sub` holding **no** `gym_membership` row was run against LIVE and returned **all 4 gyms, each owner's `auth.users` UUID, and one `legal_name`** — N rows at N customers, not zero. The tell is inside this very bullet: it counted the **25 scoped** SELECT policies and never counted the **unscoped** one. `gym_anon_select` is `for select to anon, authenticated using (true)`, and `20260713190100_gym_anon_column_grants.sql` revoked the wide grant from `anon` and stopped one role short of `authenticated`. Closed by **#213**; the reason nobody noticed for 27 days is **#214** (the anon-read axis had no machine guard). The *first three* sentences of this bullet stand — no policy reads a header, and that is still true.
