...
// biome kit per realm: [groundBase, groundDapple, canopy, canopyShade, fieldStripe]
const BIOME = {
  foundation:       ['grass', 'grass2', 'orchard', 'orchard2', 'farmcoral'],
  'sellable-product': ['springgrass', 'springgrass2', 'springtree', 'springtree2', 'flowerwhite'],
  monetization:     ['autumngrass', 'autumngrass2', 'autumntree', 'autumntree2', 'dirt'],
  'growth-reach':   ['taigagrass', 'taigagrass2', 'pine', 'pine2', 'stone'],
  'go-to-market':   ['harvestgrass', 'harvestgrass2', 'olive', 'olive2', 'sand'],
  // support's field stripe is 'stone', not a white: pure white rows on teal
  // were the loudest thing on the map after the cloud deck and pulled the eye
  'customer-support': ['mistgrass', 'mistgrass2', 'mistytree', 'mistytree2', 'stone'],
  'latam-expansion': ['grass', 'grass2', 'orchard', 'orchard2', 'farmcoral'], // 0% earned: never shown
}
// what each realm is MADE of, not just tinted with. [treeBias, fieldBias, flowers]
const DENSITY = {
  foundation:         [1.00, 1.30, false],
  'sellable-product': [1.05, 0.55, true],
  monetization:       [1.45, 0.35, false],
  'growth-reach':     [1.70, 0.15, false],
  'go-to-market':     [0.45, 2.10, false],
  'customer-support': [0.90, 0.45, true],
  'latam-expansion':  [1.00, 0.50, false],
}
...
const BORDERS = [418, 354, 290, 226, 162]
const borderV = (i, u) =>
...
function bandAt(u, v) {
  if (v < chanN(u)) return 'latam-expansion'
...
// keep seats as (u, v) of the model footprint CENTRE
const SEAT_UV = {
  foundation:       [62, 448],
  'sellable-product': [-48, 384],
  monetization:     [44, 320],
  'growth-reach':   [-66, 256],
  'go-to-market':   [38, 192],
  'customer-support': [-52, 128],
  'latam-expansion': [10, 44],
}
const FLIPS = { monetization: true, 'go-to-market': true, 'customer-support': true }
const seatWD = (id) => (id === 'latam-expansion' ? [21, 14] : [30, 20])
...
function buildWorld(realmOf) {
  ...
  const waypoints = [[104, 476], doorUV('foundation')]
  const PASS = {}
  ...
  for (let y = 0; y < GH; y++) {
    for (let x = 0; x < GW; x++) {
      ...
      TOP[i] = BIOME[JOURNEY[BAND[i] - 1]][0]
    }
  }
  ...
  for (let y = 1; y < GH - 1; y++) {
    for (let x = 1; x < GW - 1; x++) {
      ...
      const kit = BIOME[JOURNEY[BAND[i] - 1]]
  ...
  for (const id of JOURNEY) {
    ...
    const origin = id === 'foundation' ? doorUV('foundation') : PASS[id]
...
export function composeOverworld(realms, opts = {}) {
  ...
  if (!Array.isArray(realms) || realms.length !== JOURNEY.length) {
    throw new Error(`composeOverworld: expected ${JOURNEY.length} realms, got ${Array.isArray(realms) ? realms.length : typeof realms}`)
  }
  realms.forEach((r, i) => {
    if (!r || r.id !== JOURNEY[i]) {
      throw new Error(`composeOverworld: realm ${i} must be '${JOURNEY[i]}', got '${r && r.id}'`)
    }
  })
  ...
  for (const id of JOURNEY) {
    ...
    if (realmOf(id).pct === 100) {
      const model = castle.MODELS[id === 'latam-expansion' ? 'castle-24' : 'castle-32']
      keep = model.build(0)
  ...
  for (const id of JOURNEY) {
    ...
    const deep = id === 'latam-expansion' ? 0.93 : 0.5
  ...
  {
    const [csx, csy, cw, cd] = seats['customer-support']
  ...
  for (const id of JOURNEY) {
    ...
    const ly = frame.sy(sx + w - 1, sy + d - 1, ZTOP) + (id === 'latam-expansion' ? 15 : 3)
