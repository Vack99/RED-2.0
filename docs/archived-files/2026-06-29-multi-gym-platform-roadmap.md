...
| 3 | **Tenant/identity foundation** *(data track)* | `gym`, `gym_membership(user, gym, role)`, `member` (evolve `clientes` + nullable `auth_user_id`); **member self-register + claim-by-match**; RLS migrated per-`auth.uid()` → **gym-scoped + role**. *(Catalog/booking tables land in Phases 5/6 — see data-model doc §4.)* | 2 | Forge admin ops pass under gym-scoped RLS; **cross-gym denial test** (extends `rls_cross_tenant_denial.sql`) green; a member can self-register, **claim** their pre-created row, and read only their gym's data; **host→gym resolution reads the `gym`/`gym_domain` rows (brand derived from the gym row) and the `HOST_TO_BRAND` stub is deleted** (ADR-0012 §5); **custom SMTP live for auth mail + auth email templates verified** (Supabase's default sender is dev-only — single-digit emails/hour); per-gym `timezone` threaded through `@gym/format` + the `toggle_pase` RPC. |
| 4 | **Brand system** *(frontend track)* | Full `packages/brand/{forge,red}`: brand-keyed CSS-var token sets (generalise `globals.css`), logo components, animation modules (`forge-*` vs RED ignition), copy; a neutral **`base` module + gym-row token-override merge** (zod-validated before serialization) — the brand-is-DATA path ADR-0012's thousands-scale mechanism rests on; **de-brand the admin shell** (title, toaster, lockups, login animation from the resolved module — 4 Forge hardcodes today) | 2 *(∥ 3 — the row-data merge exit needs 3's `gym` table)* | Either app renders fully in either brand by host, **no FOUC**, `prefers-reduced-motion` respected; a gym with **no dedicated code module** renders from `base` + row data. |
| 5 | **Admin reframe + Agenda** ✅ *(exit gate #47 passed 2026-07-05)* | The admin redesign + **catalog/scheduling schema & authoring**: `class_type`, `coach`(+multi-coach join), `class_session` (absolute `starts_at`) + `schedule_template`, `plan`, gym content — the **Agenda** page | 3, 4 | Operator schedules a `class_session` (one-off or recurring) and curates plans/coaches/content; occupancy **derived**; writes gym-scoped + *"visible en la app"*. **MET** — live forge walk + red-demo check recorded in `docs/runbooks/red-demo-seed-evidence.md` §Phase-5 exit gate. |
...
| 7 | **Harden & launch** | Member-auth security review, RLS denial tests, prod domains/SSL, per-brand QA, bundle/perf, minimal observability | 6 | Both gyms live in production; isolation verified at the DB layer; brand bundles within budget; error tracking + RPC-failure alerts + a per-gym support contact live; auth-mail deliverability verified; TTFB spot-checked from a Mexican vantage point. |
...
- **Payments / subscriptions** — UI-only for v1 ("paga en tu gym", no dead card fields); no processor. Revisit trigger: before onboarding the first non-founder-operated gym.
