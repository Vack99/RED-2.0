...
- ALWAYS: `superpowers:using-git-worktrees` (you are in an isolated worktree) + `superpowers:verification-before-completion` (CRITICAL — a green `pnpm lint && typecheck && test && build` and the slice's observable proof are the acceptance signal, NOT a claim) + `keep-it-lean` (the YAGNI shield — run the deletion test on every new Module and the no-op test on every sentence you write; the ONLY licence to add structure is a **named present need** — a second consumer or enforced boundary that exists in THIS diff).
