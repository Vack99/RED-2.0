...
Repo `C:\Users\Aaron\Documents\Repos\RED-2.0`, HEAD `33c9087a`. Live project
`hjppxawglmukfvsgmcog` (SELECT-only). Eleven territory seats, two verification lenses per seat, one
referee. **76 raw findings → 58 after merging duplicates → 3 refuted, 3 unmeasured.**
...
3. **Nothing on live *must* change this week to keep members working.** No deploy is required. Two
   ...
   and Supabase → Auth → Rate Limits (the number every capacity figure here rests on).
4. **Three RED members are wedged right now**, worst stuck 475 hours — `select * from
   public.registros_atorados()` returns 3 rows, and the alert cron that exists to page you on
   exactly that has sent **0 alert mails in the full 30-day Resend ledger (194 mails)**.
5. **The single most plausible reason failures feel "elevated":** the door mix inverted. RED
   members claimed **before** commit `afd7a5d5` (2026-08-30): 17 via `/activar`, 17 via `/registro`.
   **After**: 1 via `/activar`, 8 via `/registro` (live split on
   `auth.users.raw_user_meta_data ? 'full_name'`, re-run independently by both lenses). Every
   failure mode in this document lives on the `/registro` rail, and ~89% of new members now take it.
6. That is **regression *and* exposure** — see §2. The regression is real (n=9, so a contributor,
   not a proven cause); the exposure is that RED added 27 claimed members since 2026-08-15, so a
   constant per-member failure rate produces more failures than it used to.
...
9. **The cheapest three fixes**, all app-tier, no migration: put the send time in the mail subject;
   branch on `over_email_send_rate_limit` at `activar/actions.ts:94` and show a countdown instead of
   "NO SALIÓ EL CORREO"; route `enviarMagicLink` through the throttle the other three doors share.
...
|---|---|---|---|---|---|---|
| 1 | **Two of five mail doors bypass the one shared throttle, and the 429 renders as a lie with a retry that re-fails** | "NO SALIÓ EL CORREO — Intenta de nuevo", twice, 10 s apart | `reenvio-limite.ts:4-9` claims "ALL THREE doors"; grep gives exactly 3 call sites (`entrar/actions.ts:84`, `registro/actions.ts:97`, `registro.ts:149,177`). `activar/actions.ts:87` → `sesion.ts:179-201` is ungated, as is `entrar/actions.ts:57` (`solicitarReset`). `activar/actions.ts:90-93` collapses every failure to `cuentaExistenteFallo`; `activar-form.tsx:145-160` retries with `window.location.reload()` into the same window | held | measured — live `auth_logs`: 2 x 429 `over_email_send_rate_limit` on `/otp` at 2026-09-01 19:25:17Z and 19:25:27Z, the only two in 24 h; GoTrue's own body says "after 28 seconds", 32 s after her 19:24:45Z `/signup` = a 60 s per-address floor | Wrap `enviarMagicLink` + `solicitarReset` in `permitirReenvio`; give `over_email_send_rate_limit` its own state with a countdown. T1-03/04, T2-01, T5-02/03, T9-03, RT-06 |
| 2 | **One tap binds someone else's paid membership to whoever is signed in, and overwrites the invited email — no undo in the app** | Member B's paid month appears in member A's app; B's invite is dead and the desk's REENVIAR button refuses | `activar/page.tsx:83-85` renders `VincularForm` on *any* live session; `activar/actions.ts:137` calls `intentarReclamoPorCodigo` (which mints the firma server-side, `registro.ts:343`); `reclamar_por_codigo.sql:54-58` guards only "caller owns no row in this gym", then `:60-68` sets `auth_user_id`, `email = v_email`, `claim_code = null` in one UPDATE with no comparison to the target row's email | held | measured — RPC body read at `cat -n`; 67 armed codes live (`claim_code not null and auth_user_id is null`); `clientes` has no `updated_at`/`deleted_at` and `auth.audit_log_entries` = 0, so the original address is unrecoverable | Refuse in the RPC when the row's existing `email` is non-null and differs from the caller's verified address; show the invitee's **masked** email in `VincularForm`. Takes `test:denial` with a written-row assertion. T3-06, T4-07, T7-01, RT-07 |
| 3 | **The door mix inverted: ~89% of new RED members now enter through the rail that carries every failure** | Invited member taps LOGIN, sees "Crea tu cuenta", mints a second identity, and `/activar` is on the magic-link rail forever | `entrar-form.tsx:341-353` renders an unconditional full-width "Crea tu cuenta" to `/registro`; `registro.ts:158` stamps `full_name`, `activar-cuenta/index.ts:92-95` does not — that is the discriminator. `activar/actions.ts:80-95` routes every `email_exists` to `cuenta_existente` | held | measured — RED claimed members: before `afd7a5d5` 17/17, after 1/8; weekly 12/6, 2/6, 3/8, 1/5; 20 of 27 since 2026-08-15 carry the `/registro` signature; live 24 h `/admin/users` = 1x200 vs 5x422 `email_exists`. n=9 post-change and the trend starts 08-17, so contributor, not proven cause | At `/registro`, an email matching an unclaimed coded row in the host gym routes to `/activar` instead of signing up. T2-03, T5-01, RT-02 |
| 4 | **The only automated watcher on this path has never sent an alert, while 3 members sit wedged** | None — that *is* the defect; the operator is never told | `resumen.ts:176-179` alerts on **any** wedged row on **every** run and mails through the same Resend key; `route.ts:32` documents `0 * * * *` hourly while `apps/admin/vercel.json:5` is `0 12 * * *` (daily, since `860a3893`) | held | measured — live `registros_atorados()` = 3 rows, 475 h / 232 h / 66 h; 194-mail Resend ledger paged twice by two lenses, **0** subjects containing "Alerta"; no `pg_stat_statements` entry for `resumen.ts:86`'s literal SQL since the 2026-05-29 reset. **The cause is not settled** — "the cron never fires" vs "it fires and dies at the 401/500 env guard" are indistinguishable from here | `curl -H "Authorization: Bearer $CRON_SECRET" https://<admin-host>/api/cron/alertas` — the 401-vs-500 body names the missing variable. Then write a liveness row per run to `public.cron_run_log` (which already works). T1-01, T6-02, RT-05 |
| 5 | **Login and roster are joined on `lower(email)` alone; a miss silently mints a second, zero-balance member** | "Pagué y la app dice 0 clases", with no error and no way back | `reclamar_o_crear_cliente.sql:50-51` matches on `gym_id` + `auth_user_id is null` + `lower(email)`; `:74-86` falls through to an INSERT with `clases_restantes = 0`; the invite tap afterwards hits `reclamar_por_codigo.sql:54-58` ("Ya tienes cuenta en este gimnasio"), and `activar/actions.ts:137-138` swallows that refusal and redirects to the same empty screen | held (mechanism); **live occurrence unmeasured** | measured for the code path and the exposure — 47 rows carry a `claim_code` with **no email** (forge 24 / red-demo 11 / red 8 / forge-demo 4), all 8 RED ones with a current `vence`; 2 split-name pairs already exist (red "karen lara", `vence` {2026-09-12, 2026-07-16}; forge "joel trevizo"). No claimed row was found beside an unclaimed paid twin | Before the INSERT, refuse with `POSIBLE_DUPLICADO:<id>` when an unclaimed row in the gym shares the 10-digit-normalised phone. A refusal, not a phone claim — ADR-0009 survives. T4-01, T7-07, RT-08 |
| 6 | **8 paying RED members are structurally uninvitable and the desk shows no defect** | Member paid, has no app, and nobody knows | `derive.ts:160-174` derives `sin_email`; `cliente-detalle.tsx:487-488` gates the whole invite block on `sin_invitar` OR `invitacion_enviada`, with its own comment conceding `sin_email` "has nothing to send"; `invitaciones.ts:229` returns `{ok:false, motivo:'sin-email'}` | held | measured — RED with `vence >= current_date`: 47 vigentes, 37 con acceso, **10 sin acceso, 8 of them with no email at all**; 15 RED rows have `email IS NULL`, oldest 2026-05-20, newest 2026-09-01; 91 platform-wide | Add `sin_email` to the ficha's invite block with an "AGREGAR CORREO" action, plus a roster-level count. ~4 lines of JSX. RT-01 |
...
| 9 | **A dropped response mid-activation permanently moves the member onto a rail production has never once exercised** | Generic "No pudimos activar tu cuenta"; every retry from then on lands on the magic-link screen | `activar-cuenta/index.ts:92` `createUser` then `:105` `generateLink` then `activacion.ts:124` `confirmarTokenHash`: three uncommitted-together side effects with **no compensating `deleteUser`** on the `linkErr` path; the retry hits `email_exists` and falls to `cuenta_existente` | held | measured — live 24 h `/admin/users` 6 calls vs `/admin/generate_link` 1 (five createUser attempts that never reached generateLink); live passwordless-confirmed rows = 0; **live identities holding `clientes` rows in two gyms = 0**, so the rail's designed case has never occurred | On `linkErr`, delete the user this same invocation created; or treat a pre-existing user with no password as a RESUME. T2-05, T7-02, T9-06 |
| 10 | **The wedge detector sees 1 of 7 genuinely stuck RED members, mislabels the one it sees, and forgets everyone at day 30** | The member is invisible; nobody is told | `registros_atorados.sql:3` roots the CTE in `auth.users`, so a member who never clicks the invite has no row at all; `:31` hardcodes `filas_roster` to the literal `0` on the sin-vincular arm; both arms carry `> now() - interval '30 days'` (`:25`, `:34`) | held | measured — live `registros_atorados()` = 3 rows including iva***@gmail.com / sin-vincular / 66 h / `filas_roster: 0` while a paid roster row exists; 8 RED rows hold a live `claim_code` + email and only 2 have any `auth.users` row; oldest stuck invite ale***@gmail.com sent 2026-08-05 = 28 days; jes***@hotmail.com rolls out around 2026-09-13 | Add a third arm rooted in `public.clientes` (`auth_user_id is null and claim_code is not null and invitacion_enviada_at < now() - interval '48 hours'`); compute `filas_roster` on both arms; widen the outer bound to 365 days and cap the mail body instead. RT-04, T6-03, T1-01 |
...
| 26 | **The only browser gate on this surface has been red since 2026-08-30, and an unarmed run exits 0** | None; it is why three auth-surface pushes shipped unproven | `playwright.config.ts:68` baseURL is `http://red-demo-client.localhost:3100`, a host GoTrue clamps to the bare Site URL, so `correo.ts:64-77` throws, `index.ts:106-111` returns 400, and GoTrue reports "500: Invalid payload sent to hook". `session.spec.ts:63-64` is a literal `test.skip` on unset credentials; neither `.github/workflows/ci.yml` nor `.husky/pre-commit` runs `test:e2e` | held, severity cut 4 to 3 on scope | measured — live 24 h: exactly **6** `unexpected_failure` 500s on `/signup`, all `delivered@resend.dev`, all from the owner's dev IP 189.237.183.74, while every real member's `/signup` in the same window returned 200 with a matching Resend mail (alv*** 01:28:34, and*** 21:56:43, oma*** 21:56:28, Marce 19:24:45). It is a **latent config trap and a dead gate**, not a live member failure | Add the Playwright host to Supabase's Redirect URLs (or pass an already-allowed `emailRedirectTo`), and make an unarmed `test:e2e` exit non-zero. RT-09, T8-12, T5-05 |
...
**Verdict up front: both regression and exposure, and the regression is concentrated in one
commit-day.**
...
- **Exposure:** RED claimed **27 new members since 2026-08-15** (live `auth.users` joined to
  ...
  members in 2.5 weeks than it did before. **measured.**
- **Regression:** the completion split moved at the 2026-08-30 cut. `/activar` vs `/registro` among
  RED's claimed members: **antes 17/17, después 1/8**; weekly 12/6, then 2/6, then 3/8, then 1/5.
  **measured**, n=9 post-change — a **contributor, not a proven cause**, and the trend already
  starts 08-17.
- **Copy regression:** before `afd7a5d5`, a throttled magic-link send rendered "Revisa tu correo";
  after it, the same 429 renders a full-screen "NO SALIÓ EL CORREO" whose only remedy re-fails.
  **measured** (`git show afd7a5d5 -- apps/client/src/app/activar/actions.ts`).
...
|---|---|---|---|
| **Resend account daily cap** | **50 new members/day, platform-wide** (100 mails divided by 2 per new-client sale) | Two concurrent sends per sale (`vender/actions.ts:61-71`), no queue, no retry, no outbox; the refusal reason is discarded at `:114` | **YES — first to bind.** The 100/day input is *asserted* (2026-07-22 audit); the 2-per-sale and the 37/day measured peak are *measured* |
...
**The headline number: 50 new members per day, platform-wide, and the operator cannot see the wall.**
It rests on one asserted input — the Resend plan. If the account is already on Pro, the first binding
component becomes the GoTrue auth-email bucket at a documented 30 new users/hour.
...
| **`realtime.messages` daily partitions** | **4 consecutive days with zero client subscribes** — the service pre-creates only yesterday through today+3 | Nothing visible: `realtime.send` swallows the failure (`EXCEPTION WHEN OTHERS THEN RAISE WARNING`, verified in the live function body), so pages simply stop being fresh | No, by design — and keeping the swallow is correct, since a broadcast must never roll back a sale |
...
| Keep | Exit trigger |
|---|---|
| Supabase GoTrue as the auth provider — nothing found here is a GoTrue defect; all 30 ranked rows are a door, a counter or a screen this repo owns | The project-wide auth-mail bucket refuses **more than 5 sends in any 7-day window** attributable to distinct real members |
...
| `pnpm test:e2e` as a convention rather than CI — it genuinely needs a real auth server | Make it blocking after **2 consecutive auth-surface pushes** land without a green run. HEAD is at **3 or more** (senal, modos, glance-card). **ALREADY TRIPPED** |
| `registros_atorados()` — it found iva*** | Its coverage of stuck RED members staying **below 50%** after a `clientes`-rooted arm is added. Today: **14%**, 1 of 7 |
| Resend as the single mail transport (DKIM and both SPF records verified live; 190 of 194 delivered) | A real-recipient bounce rate over **2.0%** in any rolling 30 days (today 0.52%, 1 in 191), **or** any single day over **80 mails** (today's peak 37) |
...
| **The alert cron in its current state — DO NOT KEEP.** Zero output in 30 days against a 475-hour wedge; counting it as coverage is worse than having none | **undecided** — are `CRON_SECRET`, `SUPABASE_ACCESS_TOKEN` and `ALERT_EMAIL` set on the admin Vercel project, and is the cron scheduled there? **Only the owner can read that dashboard** |
...
- The whole `/registro` vs `/activar` split (17/17 to 1/8; weekly 12/6, 2/6, 3/8, 1/5; 20 of 27 since
  08-15) — two independent live re-derivations.
...
- All live table counts: `registros_atorados()` = 3 rows (475/232/66 h); 67 armed codes; 10 claimed
...
- The Resend ledger: 194 mails, 08-04 to 09-02, paged to `has_more:false` by two lenses; subject
  split 45/2; per-address stacks 6/6/4/3/2; 3 bounces plus 1 delay, 3 of the 4 on `@red-demo.test`;
  two sender display names 119/57; **0** alert mails.
...
- **50 new members/day platform-wide** = 100 mails/day divided by 2 measured concurrent sends per
  sale. The 100/day is *asserted*.
...
- Resend Free equals 100 mails/day (2026-07-22 capacity audit, P-069 / P-105).
...
| Did the 2026-08-30 `&correo=` cut actually raise the `email_no_coincide` rate? | Nothing logs it. Add a line at `activar/actions.ts:73` and count for 30 days. The current activation-share comparison (n=8) establishes nothing |
...
| Do `pnpm test`, `pnpm test:denial` and `pnpm test:e2e` pass at HEAD? | Run them. **None was run this round** — see §14 |
...
1. **"Is the Vercel cron for `/api/cron/alertas` scheduled and enabled on the admin project, and what
   did its last invocation return?"** This is the single highest-value read in the document — it
   converts §1 row #4 from "silent, cause unproven" into a named cause.
...
4. **"What plan is the Resend account on, and what is the real daily quota?"** Turns §4's headline —
   50 new members/day, platform-wide — from modelled into measured.
...
11. **OWED INPUT, still outstanding: the `gym_legal` content for RED and Forge.** The table is empty
    for all four gyms, which is why 51 of 51 consent stamps are versionless.
12. **OWED INPUT, carried from prior sessions: SAT persona-física details.** Unrelated to this
...
- **No gate was run this round** — no `pnpm test`, no `pnpm test:denial`, no `pnpm test:e2e`, no
  build, no lint. Every "the tests stay green" claim in §7 is derived from **reading** assertions,
  config, `.github/workflows/ci.yml` and `.husky/pre-commit` at HEAD, never from executing anything
...
- **No browser, no device, no Playwright.** Every tap sequence is derived from source. Turnstile's
...
- **Forge's own funnel.** Forge holds 28 live claim codes, more than RED, with
  `booking_enabled:false`. Its mail was measured; its member journey was not. A fourth live gym,
...
**Statistical caveat:** every rate in this document has a denominator between 5 and 194 over a single
