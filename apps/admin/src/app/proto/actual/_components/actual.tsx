...
// apps/admin/src/app/proto/actual/_components/actual.tsx — THE BASELINE.
//
// A verbatim copy of apps/admin/src/app/(app)/clientes/_components/clientes.tsx
// (ClientesScreen), rendered against the shared fixture roster instead of a
// live DB read. Do NOT fix anything here — this is the control every other
// /proto variant is judged against, and it must reproduce the complaint
// faithfully: the diasRest-ascending sort with no floor, urgenciaCliente with
// no floor, the --red/--gold accent, the non-partitioning header. The only
// changes from the original are plumbing, never behavior:
//   - the prop type is RosterClient[] (the fixture roster), not a live
//     ClienteRosterDTO[] fetch — RosterClient IS a ClienteRosterDTO plus two
//     unused-here fields, so every line below reads identically;
//   - the in-app-nav breadcrumb (sessionStorage, real-app-only plumbing) is
//     dropped from the row Link's onClick — irrelevant to the thing being
//     measured and there is no ficha route under /proto to return from;
//   - wrapped in ProtoShell instead of the real (app)/layout.tsx, which is
//     the same frame + tab bar under a different file (hard rule 1: nothing
//     under proto/ may import from (app)/).
// Everything else — every threshold, sort, filter, and pixel — is unchanged.
...
export function ActualScreen({ clientes }: { clientes: RosterClient[] }) {
