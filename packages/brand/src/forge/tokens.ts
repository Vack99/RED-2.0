/**
 * Forge's pre-serialized `:root` + `.dark` token block — the brand's fill of the
 * `@gym/ui` CSS-variable contract, ready to SSR-inline as a `<style>` (ADR-0012 §3).
 * These are the Forge token VALUES relocated out of `apps/admin/globals.css` so
 * Forge is defined once; admin keeps its raw block transitionally until S3 swaps it
 * for this inline (behaviour-preserving — the values are CSS-equivalent).
 *
 ...
 */
export const forgeTokenCss = `:root,
.light {
  --canvas: #f4f2ed;
  --surface: #ffffff;
  --sunk: #e8e5de;
