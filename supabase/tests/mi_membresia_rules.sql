...
begin
  ...
  -- gym-midnight — anchor gym-tz day = 2026-06-14. (Contract-B dropped user_id; the tenant key is gym_id.)
  -- folio is per-gym now (no sequence default) — explicit distinct values for the fresh test gym.
  insert into public.ventas (gym_id, cliente_id, folio, paquete_nombre, clases, vigencia_tipo, vigencia_dias, monto, metodo, fecha) values
    (gym_t, c_a, 9001, '8 clases', 8, 'dias', 30, 700, 'efectivo', '2026-05-01 12:00:00+00'),
    (gym_t, c_a, 9002, '8 clases', 8, 'dias', 30, 800, 'efectivo', '2026-06-15 05:30:00+00');
  ...
  -- member_b sale: ilimitado / vigencia 'mes', monto 1400.
  insert into public.ventas (gym_id, cliente_id, folio, paquete_nombre, clases, vigencia_tipo, vigencia_dias, monto, metodo, fecha) values
    (gym_t, c_b, 9003, 'Ilimitado', null, 'mes', null, 1400, 'transferencia', '2026-06-10 12:00:00+00');
  insert into public.asistencias (gym_id, cliente_id, fecha, consumio, deleted_at) values
