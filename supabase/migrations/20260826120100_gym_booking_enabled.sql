-- FORGE CONTAINMENT: a per-gym switch that turns member booking OFF (2026-08-26 "clases restantes"
-- drift audit). Forge is a class-only gym whose members do not book through the app at all — every
-- attendance is a roster mark — so every `reservar_clase` call that lands there is either a mistake
-- or a probe, and each one takes a HOLD (charge-at-booking, #233) against a balance nobody is
-- watching. The audit's realized damage at forge was exactly that: burned holds on a gym that does
-- not use the feature. This closes the door at the gym, not at the member.
--
-- SHAPE: one boolean on `gym`, defaulting TRUE — every existing gym keeps booking, and a new gym
-- gets it. Only forge is flipped, by SLUG (stable, and the same key every seed migration uses); the
-- statement is a no-op wherever that gym does not exist, so it is safe on a scratch project.
--
-- COLUMN GRANT, not just a policy: `authenticated` holds COLUMN-level select on gym (20260802120000
-- revoked the table grant), so a new column is invisible to the member app until it is granted here
-- — the member seam reads it through `resolverMiembroGym`'s gym embed. `anon` is deliberately NOT
-- granted: the pre-auth host->brand lookup has no booking surface to gate.
alter table public.gym
  add column booking_enabled boolean not null default true;

comment on column public.gym.booking_enabled is
  'Member-facing booking switch. false = reservar_clase refuses for this gym and the member app hides its booking CTAs (class-only gyms whose attendance is roster-marked).';

grant select (booking_enabled) on public.gym to authenticated;

update public.gym set booking_enabled = false where slug = 'forge';

-- ══════════════════════════════════════════════════════════════════════════════════
-- reservar_clase — ONE new gate, beside the gym's other server-authoritative reads.
-- ══════════════════════════════════════════════════════════════════════════════════
-- Replayed forward from 20260805110000 with a single change: the `select timezone` that already
-- fetches this gym's row now also fetches `booking_enabled` and refuses when it is false. Placing it
-- there costs no extra round trip and puts the refusal next to the other gym-derived gates (the two
-- vence arms), which is where a reader looks for "what does this GYM say".
--
-- It sits BELOW the identity branch on purpose: the message names a gym-wide policy, and disclosing
-- it to someone who is not a member of the gym (or not staff of it) would be a probe answer. Both
-- identity paths are covered, staff included — a class-only gym has no phone-booking either, and
-- #235's rule stands (an operator blocked is an operator who marks the roster instead).
--
-- Same signature — CREATE OR REPLACE carries the EXECUTE grant, no re-issue needed.
